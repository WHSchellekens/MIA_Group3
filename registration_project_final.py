...
def intensity_based_registration_demo(I, Im, method, check, lr):
...
"""Project code for intensity image registration with a variable learning rate"""

def intensity_based_registration_variablelr(I, Im, method, check):
    ...
    # set empty arrays for registration vectors
    if method == 'rigid':
        x_array = np.full((num_iter, 3), np.nan)
    elif method == 'affine':
        x_array = np.full((num_iter, 7), np.nan)
    
    # set initial similarity
    S = 0 
    
    # perform 'num_iter' gradient ascent updates
    for k in np.arange(num_iter):

        # alter the learning rate if similarity gets above certain threshold
        if S > 0.5:
            mu = 0.001
            if S > 0.9:
                mu = 0.0001
        ...
        # save registration vector
        x_array[k] = x

        display(fig)
        
    return similarity, x_array