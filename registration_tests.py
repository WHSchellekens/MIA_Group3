...
def combining_transforms():
    ...
    X = util.test_object(1)
    X_rot_shear=reg.shear(0.1,0.2).dot(reg.rotate(3*np.pi/4).dot(X))
    fig=plt.figure(figsize=(12,5))
    ax=fig.add_subplot(141,xlim=(-4,4),ylim=(-4,4))
    util.plot_object(ax,X_rot_shear)
    #------------------------------------------------------------------#
...
def arbitrary_rotation():
    ...
    phi=np.pi/4
    xr=[-1.1,2.2]
    XR=np.array([[1,0,xr[0]],[0,1,xr[1]],[0,0,1]])
    R=np.array([[np.cos(phi),-np.sin(phi),0],[np.sin(phi),np.cos(phi),0],[0,0,1]])
    T=XR.dot(R.dot(np.linalg.inv(XR)))
    X_rot = T.dot(Xh)
